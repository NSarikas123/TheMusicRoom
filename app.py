...
from Controllers.EmailHandler import EmailHandler
from Controllers.XSSProtection import XSSProtection
from Controllers.InputValidator import InputValidator
from Controllers.TokenHandler import TokenHandler
from Controllers.SQLInjectionProtection import SQLInjectionProtection
app = Flask(__name__)
...
def register():
	if request.method == "POST":
		...
		if password1 == password2 and len(password1)>9 and usernameIsUnique == True and len(username) > 0:
			#get user input
			firstName = (request.form['firstname']) #unsafe
			surName = (request.form['surname'])	#unsafe
			userName = (request.form['username']) #unsafe
			password = (request.form['password1']) #unsafe
			email = (request.form['email'])	#unsafe		
			#validate user input
...
def banUser():
	if request.method == "POST":
		...
		if isAdmin == True:
			...
			database.ban_user(bannedUserName, cursor)
